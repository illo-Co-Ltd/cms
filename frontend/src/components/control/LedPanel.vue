<template>
    <section>
      <div id = "ledArea" class = "test">
        <div class="displayControlTogleBtnArea">
            <h3 class = "btnText">LED</h3>
            <img src="@/assets/icon/icon_home.png" v-on:click="DisplayTogleEvent('ledArea')"> <!-- 토글 버튼 -->
        </div>
        <div class="displayControlTogleMainArea">
          <input class = "rangeInput" id="ledValue" type="range" name="points" min="0" max="255" step="0" value="0" v-on:input="RangeValueInputEvent('ledArea'), LEDInsert()">
  ...
  setup(props){
    ...
    const LEDInsert = () =>{
      ai.put("/control/led", {
        ...
        value : document.getElementById("ledValue").value,
      }).then(res => {
          console.log(res);
      })
    };
